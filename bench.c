 ...
 * realworld usage.
 */
static const char *msg_dum = "GET / HTTP/1.1\r\nHost: 127.0.0.1\r\n\r\n";
...
static void *bench_worker(__attribute__((unused)))
{
    ...
    if (strncmp(dummy, "HTTP/1.1", 8) != 0) {
        puts("HTTP response validation failed");
        exit(-1);
    }
...
static void bench(void)
{
    for (int i = 0; i < BENCH_COUNT; i++) {
        ...
        /* all workers are ready, let's start bombing khttpd */
        pthread_cond_broadcast(&worker_wait);

        pthread_mutex_unlock(&worker_lock);

        /* waiting for all threads to finish the measurement */
